...
const updateShop = asyncHandler(async (req, res) => {
  ...
  if (files.bannerImage) {
    ...
    if (shop.bannerImage.url) {
      const oldBannerPublicId = shop.bannerImage.url
        .split('/')
        .pop()
        .split('.')[0];
  ...
  shop.bannerImage.altText =
    info.bannerImageAltText || shop.bannerImage.altText;
...
const getAllShops = asyncHandler(async (req, res) => {
  const shops = await shopModel.find({}).populate('owner').populate('products');
  return res.status(200).json({
    success: true,
    shops,
  });
